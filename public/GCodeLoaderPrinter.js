 ...
 */
class GCodeLoaderPrinter extends Loader {
  constructor(manager) {
    super(manager);
  ...
  parse(data) {
    ...
    function newLayer(line) {
      currentLayer = { vertex: [], pathVertex: [], supportVertex: [], z: line.z };
    ...
    function addSegment(p1, p2, tc) {
      ...
      if (state.extruding || state.f === 1800) {
        ...
        if (tc === 1) {
          const offset = 0;
                  currentLayer.supportVertex.push(p1.x, p1.y, p1.z + offset);
        currentLayer.supportVertex.push(p2.x, p2.y, p2.z + offset);
          printerPath.push({
            name: "extruded",
            p1x: p1.x,
            p1y: p1.y,
    ...
    if (this.splitLayer) {
      for (let i = 0; i < layers.length; i++) {
        ...
        addObject(layer.pathVertex, false, i);
                addObject(layer.supportVertex, false, i);
      }
    } else {
      const vertex = [],
        pathVertex = [],
      ...
      addObject(supportVertex, true, layers.length);
